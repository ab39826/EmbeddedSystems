// Adapted from PWM.c
// Runs on LM3S1968
// Use PWM0/PG2 to generate pulse-width modulated outputs
// Anurag Lubars and Joseph Banerjee
// June 27, 2011
...
#define PWM_ENABLE_PWM2EN				0x00000004  // PWM1 Output Enable
...
// PWM clock rate = processor clock rate/SYSCTL_RCC_PWMDIV
void PWM0_Init(unsigned short frequency, unsigned short duty){
  unsigned long delay;
	unsigned short period;
	period = 25000000 / frequency;
  
	SYSCTL_RCGC0_R |= SYSCTL_RCGC0_PWM;   // 1)activate PWM
	  SYSCTL_RCGC2_R |= SYSCTL_RCGC2_GPIOH; // 2)activate port H
	  delay = SYSCTL_RCGC2_R;          // allow time to finish activating
		GPIO_PORTH_AFSEL_R |= 0x01;      // enable alt funct on PH1
		GPIO_PORTH_DEN_R |= 0x03;        // enable digital I/O on PH1 and PH0
    ...
    SYSCTL_RCC_R += SYSCTL_RCC_PWMDIV_2;  //    configure for /2 divider
		...
		PWM_1_CTL_R |= PWM_X_CTL_ENABLE; // 7) start PWM1
		PWM_ENABLE_R |= PWM_ENABLE_PWM2EN;    // enable PWM2
}
